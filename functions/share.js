/**
 * GorkhaReels Share Preview - Cloudflare Pages Function
 * Path: functions/share.js
 * Intercepts /share requests and generates dynamic OpenGraph meta tags
 */
...
async function fetchReel(reelId, apiKey) {
  const response = await fetch(
    `${APPWRITE_CONFIG.endpoint}/databases/${APPWRITE_CONFIG.databaseId}/collections/${APPWRITE_CONFIG.collections.reels}/documents/${reelId}`,
    {
      headers: {
...
async function fetchCreator(creatorId, apiKey) {
  const response = await fetch(
    `${APPWRITE_CONFIG.endpoint}/databases/${APPWRITE_CONFIG.databaseId}/collections/${APPWRITE_CONFIG.collections.creators}/documents/${creatorId}`,
    {
      headers: {
        'X-Appwrite-Project': APPWRITE_CONFIG.projectId,
        'X-Appwrite-Key': apiKey
...
<body>
  ...
  <script>
    // Auto-redirect real users (not crawlers) to the video
    setTimeout(() => {
      const isCrawler = /bot|facebook|whatsapp|twitter|telegram|linkedin|slack|discord/i.test(navigator.userAgent);
      if (!isCrawler) {
        window.location = '${escapeHtml(videoUrl)}';
      }
    }, 1500);
  </script>
</body>
